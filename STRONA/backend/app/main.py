...
from . import (achievements, auth, billing, catalog, certshot, contentbot, countries,
               ...
               poller, provisioning, push, rules, telegram, telemetry, trackexport,
               tradebot)
...
@app.post("/api/admin/payouts/{payout_id}/post", dependencies=[Depends(auth.require_admin)])
def admin_payout_post(payout_id: int, request: Request):
    """Publikuje na kanale wypłatę, która JUŻ istnieje w bazie.

    Luka, którą to zamyka: `/api/admin/payout-engine/run` zawsze TWORZY nową
    wypłatę. Gdy rekord jest, a post nie wyszedł — bo bot stracił uprawnienia
    do kanału, padła usługa zrzutu albo Telegram odrzucił zdjęcie — jedynym
    wyjściem było wygenerowanie kolejnej wypłaty, czyli zmyślenie zdarzenia,
    żeby naprawić wysyłkę. Teraz publikuje się to, co faktycznie się wydarzyło.

    Bez certyfikatu nie ma czego publikować: post to zrzut strony `/payout/{token}`.
    """
    session = SessionLocal()
    try:
        p = session.get(Payout, payout_id)
        if not p:
            raise HTTPException(404, "Payout not found")
        if not p.cert_token:
            raise HTTPException(400, "This payout has no certificate yet — "
                                     "generate one first, the post is a shot of it")
        konto = session.get(Account, p.account_id)
        nazwa = (konto.trader_name if konto else "") or ""
        wynik = payoutbot.opublikuj(p, nazwa, base_url=_public_base(request))
    finally:
        session.close()
    if not wynik.get("posted"):
        # Powód prosto od Telegrama — bez niego panel mówi „nie poszło", a admin
        # szuka przyczyny w logach hostingu.
        raise HTTPException(502, f"Telegram refused: {wynik.get('reason') or 'unknown'}")
    # Zasięg dokupuje jap-tg-boost na webhooku `channel_post`, więc tutaj nie ma
    # czego wołać — patrz reach.po_publikacji i decyzja o jednym kupującym.
    return wynik
...
def _post_dict(p: ChannelPost) -> dict:
    return {"id": p.id, "channel": p.channel, "kind": p.kind, "body": p.body,
            ...
            "status": p.status, "scheduled_for": p.scheduled_for.isoformat() if p.scheduled_for else None,
...
def admin_channel_posts(channel: str | None = None, status: str | None = None):
    ...
    try:
        ...
        return [_post_dict(p) for p in q.order_by(ChannelPost.id.desc()).limit(200).all()]
    finally:
        session.close()


def _kto_z_naglowka(authorization: str | None) -> str:
    """Mail admina z tokenu sesji, o ile logował się kontem.

    `require_admin` przyjmuje też statyczny `X-Admin-Token`, przy którym nie ma
    żadnego konta do wskazania — stąd „admin" jako uczciwy zapas zamiast
    zmyślonego autora.
    """
    if not authorization or not authorization.lower().startswith("bearer "):
        return "admin"
    tid = auth.parse_token(authorization.split(" ", 1)[1].strip())
    if not tid:
        return "admin"
    session = SessionLocal()
    try:
        tr = session.get(Trader, tid)
        return (tr.email if tr else "admin")
    finally:
        session.close()
...
def admin_channel_post_create(payload: ChannelPostIn,
    ...
    Nie da się utworzyć posta od razu zatwierdzonego: zatwierdzenie jest
    momentem, w którym człowiek bierze odpowiedzialność za treść, i musi być
    osobnym kliknięciem, a nie polem w formularzu.
    """
    ...
    try:
        p = ChannelPost(channel=payload.channel, kind=payload.kind,
                        ...
                        created_by=_kto_z_naglowka(authorization))
        session.add(p)
        session.commit()
        return _post_dict(p)
    finally:
...
def admin_channel_post_approve(post_id: int):
    ...
    X, a post zakłada Y”), bo to jest informacja dla piszącego, a nie błąd
...
@app.get("/api/admin/channels/status", dependencies=[Depends(auth.require_admin)])
def admin_channels_status():
    """Zdrowie botów i kanałów Telegrama — po jednym wierszu na kanał.

    Powstało po zamrożeniu konta we wrześniu 2026, które odebrało botowi
    uprawnienia we wszystkich kanałach naraz. Objaw był najgorszy z możliwych:
    cisza. Payout BOT „przebiegł", panel nie pokazał błędu, a posty po prostu
    nie wychodziły — bo nikt o uprawnienia nie pytał.

    Pytamy `getChatMember`, nie `getChat`. `getChat` na kanale PUBLICZNYM udaje
    się każdemu botowi, także takiemu bez żadnych uprawnień, więc odpowiedziałby
    „w porządku" na dokładnie tę awarię, którą ma wykrywać.

    Nigdy nie rzuca: sprawdzenie zdrowia, które samo się wywraca, jest gorsze
    niż jego brak — panel przestałby się otwierać razem z Telegramem.
    """
    kanaly = [
        ("payouts", "Payouts", settings.telegram_chat_id, "content",
         "Payout BOT publikuje tu certyfikaty wypłat"),
        ("mgmt", "Account Management", settings.telegram_mgmt_chat_id, "content",
         "Kolejka treści i licznik miejsc w opisie kanału"),
        ("trackrecord", "Track Record", settings.telegram_trackrecord_chat_id, "content",
         "Plakaty odświeżane przez tg-graphics (repo forexpassing.com)"),
        ("leads", "LEADS", settings.telegram_leads_chat_id, "leads",
         "Karty leadów z przyciskami"),
        ("leads_ng", "LEADS Nigeria", settings.telegram_leads_ng_chat_id, "leads_ng",
         "Karty leadów z numerem nigeryjskim"),
    ]
    out = []
    for klucz, tytul, chat_id, desk, po_co in kanaly:
        bot = telegram.desk(desk)
        # Każdy kanał w osobnym `try`: jeden padnięty odczyt nie ma prawa
        # zabrać całej listy, bo wtedy awaria Telegrama zamyka panel.
        try:
            ja = telegram.get_me(bot.token) if bot.token else {}
            status = (telegram.chat_member_status(chat_id, ja.get("id"), token=bot.token)
                      if (chat_id and ja.get("id")) else "")
            opis = telegram.get_chat(chat_id, token=bot.token) if chat_id else {}
        except Exception as e:  # pragma: no cover - sieć
            print(f"[channels] {klucz}: {e}")
            ja, status, opis = {}, "", {}
        out.append({
            "key": klucz, "title": tytul, "purpose": po_co,
            "chat_id": chat_id, "desk": desk,
            "configured": bool(bot.token and chat_id),
            "bot_username": ja.get("username") or "",
...
@app.get("/api/admin/export/track-record", dependencies=[Depends(auth.require_admin)])
def admin_export_track_record(format: str = "zip", account_id: int | None = None,
                              limit: int = trackexport.LIMIT_TRANSAKCJI):
    """Zrzut track recordu kont prowadzonych przez Trade BOT — do pliku.

    Panel pokazuje liczby, żeby na nie spojrzeć; to jest zrzut do PRZETWARZANIA,
    arkuszem albo modelem językowym. Dlatego wychodzi z niego nie sama tabela,
    tylko tabela ze słownikiem: każde pole ma jednostkę i zdanie o tym, jak
    zostało policzone. Bez tego `pnl`, `drawdown` i `win_rate` są dla modelu
    wieloznaczne i wnioski wychodzą z niewłaściwego założenia.

    `zip`  — CSV-ki (otwierają się w Excelu dwuklikiem, BOM dla polskich znaków),
             JSON i `CZYTAJ-TO-NAJPIERW.md` ze słownikiem. Wariant domyślny.
    `json` — jeden plik, gdy zrzut leci prosto do modelu.
    `csv`  — sama tabela kont, gdy potrzebny jest szybki przegląd.

    Limit transakcji na konto jest twardy z powodu 60-sekundowego limitu funkcji
    na Vercelu; obcięcie jest raportowane w `trades_truncated`, nie przemilczane.
    """
    if format not in ("zip", "json", "csv"):
        raise HTTPException(400, "format must be zip, json or csv")
    session = SessionLocal()
    try:
        dane = trackexport.zbierz(session, account_id=account_id,
                                  limit=max(1, min(limit, trackexport.LIMIT_TRANSAKCJI)))
    finally:
        session.close()
    if account_id is not None and not dane["accounts"]:
        raise HTTPException(404, "No bot-driven account with that id")

    znacznik = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M")
    nazwa = f"track-record-{znacznik}"
    if format == "json":
        return Response(trackexport.do_json(dane), media_type="application/json",
                        headers={"Content-Disposition": f'attachment; filename="{nazwa}.json"'})
    if format == "csv":
        return Response(trackexport.do_csv(dane), media_type="text/csv",
                        headers={"Content-Disposition": f'attachment; filename="{nazwa}.csv"'})
    return Response(trackexport.do_zip(dane), media_type="application/zip",
                    headers={"Content-Disposition": f'attachment; filename="{nazwa}.zip"'})
...
async def api_tick(request: Request):
    ...
    # Kolejka tresci na kanaly: JEDEN zalegly post na przebieg. Kolejka
    # zostawiona na tydzien wysypalaby osiem postow w poltorej minuty, co na
    # kanale wyglada jak awaria, a nie jak publikacja.
    tresc = _content_tick()
    if isinstance(wynik, dict):
        return {**wynik, "daily_recap": recap, "upsell_nudge": nudge.get("sent", 0),
                ...
                "lead_followups": leady.get("sent", 0), "reach": zasieg,
                "channel_posts": tresc.get("sent", 0)}
...
def _content_tick() -> dict:
    """Zaplanowane posty na kanaly. Best-effort — cron nie moze paść przez treść."""
    session = SessionLocal()
    try:
        return contentbot.wyslij_zaplanowane(session)
    except Exception as e:  # pragma: no cover - siec/baza
...
def admin_lead_delete(lead_id: int):
    ...
    try:
        ...
        if not lead:
            raise HTTPException(404, "Lead not found")
        # Desk czytany razem z resztą, PRZED close(): karta wisi na konkretnym
        # czacie konkretnego bota i cudzym tokenem nie da się jej zdjąć.
        mail, karta_mid, karta_desk = lead.email, lead.tg_message_id, (lead.desk or "leads")
    ...
    # cofa kasowania.
    if karta_mid:
        telegram.delete_lead_card(karta_mid, bot=telegram.desk(karta_desk))
...
@app.post("/api/telegram/webhook/{desk}")
async def telegram_webhook(request: Request, desk: str = "leads",
    ...
    DESK SIEDZI W ADRESIE i to nie jest ozdoba. Update z Telegrama nie niesie
    ŻADNEJ informacji o tym, który bot go dostał — ani w ciele, ani w nagłówku.
    Odkąd deski leadów mają osobne boty, jedyne, co je rozróżnia, to URL, pod
    który każdy bot ma ustawiony swój `setWebhook`. Zgadywanie po sekrecie
    (próbowanie po kolei wszystkich) byłoby gorsze: cicho psuje się przy
    rotacji i przy dwóch deskach z tym samym sekretem.

    Ścieżka bez desku zostaje jako alias na desk domyślny — stary `setWebhook`
    może być jeszcze w locie w chwili wdrożenia i nie ma prawa dostać 404.
    ...
    """
    if desk not in telegram.DESKI_LEADOW:
        raise HTTPException(404, "Unknown desk")
    bot = telegram.desk(desk)
    if not bot.secret or not x_telegram_bot_api_secret_token or \
            not secrets.compare_digest(x_telegram_bot_api_secret_token, bot.secret):
        raise HTTPException(401, "Unauthorized")

    update = await request.json()
    if (update or {}).get("callback_query"):
        return _telegram_przycisk(update["callback_query"], bot)
    ...
    if ((wiadomosc.get("chat") or {}).get("type") == "private"
        ...
        return _telegram_start(wiadomosc, bot)
    if wiadomosc.get("reply_to_message"):
        return _telegram_notatka(wiadomosc, bot)
    return {"ok": True}


def _telegram_start(wiadomosc: dict, bot: telegram.Bot) -> dict:
    ...
    telegram.send_dm(czat, f"Linked as {email}. Your clicks on the LEADS channel "
                           "now sign with this account.", bot=bot)
    return {"ok": True}
...
def _telegram_przycisk(cb: dict, bot: telegram.Bot) -> dict:
    ...
    try:
        ...
        if not lead:
            telegram.answer_callback(cb.get("id", ""), "Nie znaleziono leada", bot=bot)
            return {"ok": True}
    ...
    telegram.answer_callback(cb.get("id", ""), dymek, bot=bot)
    if not zmiana:
        return {"ok": True}          # odmowa albo kliknięcie w to samo drugi raz
    wiadomosc = cb.get("message") or {}
    czat = str((wiadomosc.get("chat") or {}).get("id") or "")
    if czat and wiadomosc.get("message_id"):
        telegram.edit_lead_message(czat, wiadomosc["message_id"], tekst,
                                   keyboard=klawiatura, bot=bot)
...
def _telegram_notatka(wiadomosc: dict, bot: telegram.Bot) -> dict:
    ...
    czat = str((wiadomosc.get("chat") or {}).get("id") or "")
    if czat and mid:
        telegram.edit_lead_message(czat, mid, tekst, keyboard=klawiatura, bot=bot)
    return {"ok": True}
...
def _wyslij_zaplanowane(session, now: datetime
    ...
    Zwraca dwie listy: `(desk, tekst)` na czat działu i pushe
    ...
    Desk jedzie razem z tekstem, bo przypomnienie niesie imię i mail człowieka:
    wysłane na drugi desk byłoby wyciekiem, a nie pomyłką w adresie.
    ...
    for r in session.query(LeadReminder).filter(LeadReminder.active.is_(True)).all():
        ...
        if not lead:
            r.active = False
            continue
        teksty.append(((lead.desk or "leads"), _tekst_zaplanowanego(lead, r)))
    ...
    * `stalled`  — odebrał telefon, minęło `stalled_days` i nic z tego nie ma.
    ...
    try:
        ...
        for l in leady:
            ...
            # Desk leci razem z tekstem: przypomnienie niesie imię i mail, więc
            # wysłanie go na drugi desk jest wyciekiem, a nie pomyłką w adresie.
            do_wyslania.append(((l.desk or "leads"), _tekst_przypomnienia(l, powod, paid, dni)))
            tytul = {
    ...
    for desk_leada, tekst in do_wyslania:
        telegram.send_lead_message(tekst, bot=telegram.desk(desk_leada))
