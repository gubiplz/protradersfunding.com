...
#  Skoroszyt                                                                   #
# --------------------------------------------------------------------------- #
# Treść pliku jest po ANGIELSKU — czyta go klient, partner albo model językowy,
# a nie osoba pisząca ten kod. Komentarze zostają po polsku, jak w reszcie repo.
SLOWNIK = [
    ("Accounts", "account_login", "Platform login of the account — the key that joins every sheet", "text"),
    ("Accounts", "trader_name / trader_email", "Who the account belongs to", "text"),
    ("Accounts", "product_key", "Challenge plan the account was opened on", "text"),
    ("Accounts", "phase", "eval_1 / eval_2 = evaluation stage, funded = live account", "text"),
    ("Accounts", "initial_balance", "Starting balance of the account", "USD"),
    ("Accounts", "balance / equity", "Closed balance / equity including open position result", "USD"),
    ("Accounts", "profit_pct", "Profit against the starting balance", "%"),
    ("Accounts", "max_drawdown_pct", "Largest historical drop from the equity peak. NOTE: measured across the whole curve, so it also captures drops that are NOT trading losses — a phase pass resets the balance and a payout withdraws from it; both look like a sudden fall", "%"),
    ("Accounts", "max_drawdown_at", "When the deepest point was recorded", "date and time"),
    ("Accounts", "overall_dd_used_pct", "How much of the RULE drawdown allowance is used up (100 = the account breaches)", "%"),
    ("Accounts", "daily_loss_used_pct", "The same for the daily loss limit", "%"),
    ("Accounts", "daily_floor / overall_floor", "Equity levels at which the account breaches", "USD"),
    ("Accounts", "target_equity", "Equity that passes the phase; empty on a funded account", "USD"),
    ("Accounts", "win_rate_pct", "Share of winning trades among CLOSED trades", "%"),
    ("Accounts", "profit_factor", "Gross profit / absolute gross loss; empty = no losing trades", "ratio"),
    ("Accounts", "expectancy", "Average result of a single closed trade", "USD"),
    ("Accounts", "max_win_streak / max_loss_streak", "Longest run of winning / losing trades in a row", "trades"),
    ("Accounts", "trading_days", "Days with activity, counted by the challenge rules", "days"),
    ("Accounts", "payouts_total", "Sum of trader_share across every payout on the account", "USD"),
    ("Accounts", "bot_enabled", "TRUE = the account is driven by the trading bot, not a live MT5 feed", "true/false"),
    ("Trades", "side", "buy = long position, sell = short", "text"),
    ("Trades", "lots", "Position size", "lots"),
    ("Trades", "open_price / close_price", "Entry and exit price; exit empty while the position is open", "price"),
    ("Trades", "pnl", "Result of the position; floating while the position is still open", "USD"),
    ("Trades", "pnl_pct_of_initial", "The same result against the starting balance", "%"),
    ("Trades", "duration_min", "How long the position was held", "minutes"),
    ("Trades", "status", "closed = settled, open = still running", "text"),
    ("Daily", "day", "Trading day by server time, not the UTC calendar day", "date"),
    ("Daily", "net_pnl", "Result of the day from trades closed that day", "USD"),
    ("Daily", "day_return_pct", "Equity change within the day", "%"),
    ("Daily", "drawdown_from_peak_pct", "Distance from the highest equity up to and including that day", "%"),
    ("Daily", "snapshots", "How many equity readings back that day — few readings mean a coarse curve", "count"),
    ("Equity", "ts", "Timestamp of the equity reading (UTC)", "date and time"),
    ("Equity", "open_pnl", "Floating result of open positions at that moment", "USD"),
    ("Symbols", "net_pnl", "Result on the instrument, closed trades only", "USD"),
    ("Payouts", "profit_amount", "Profit withdrawn from the account, BEFORE the split", "USD"),
    ("Payouts", "trader_share", "The part that reached the trader after the profit split", "USD"),
    ("Payouts", "paid", "TRUE = the payout is settled", "true/false"),
    ("Breaches", "type", "Rule broken: daily_loss, max_drawdown, consistency, manual", "text"),
    ("Breaches", "equity_at_breach", "Equity at the moment the rule was broken", "USD"),
]

NOTA = [
    "WHAT THIS FILE IS",
    "",
    "One statement per account, the way a broker issues them — except every",
    "managed account is in this single workbook.",
    "",
    "It covers accounts of PAYING clients. Free signups and imported accounts",
    "are LEFT OUT: we do not run them the way we run paid ones, so their history",
    "says nothing about how the service is doing. Accounts with no owner (the",
    "provisioning pool) are left out for the same reason.",
    "",
    "HOW TO READ IT",
    "",
    "Every row on every sheet carries account_login. The account is a COLUMN,",
    "not a tab, so figures can be computed across all accounts without stitching",
    "sheets together.",
    "",
    "Statistics are computed from CLOSED trades only. An open position has a",
    "floating result, so counting it towards a win rate would score something",
    "that has not won yet.",
    "",
    "TWO NUMBERS THAT SOUND ALIKE",
    "",
    "max_drawdown_pct is the historical drop from the equity peak.",
    "overall_dd_used_pct says how much of the RULE allowance is spent — the one",
    "at which the account stops existing.",
    "",
    "max_drawdown_pct is measured across the whole equity curve, and that curve",
    "contains events which are not trading losses: passing a phase resets the",
    "balance to its starting value, and a payout withdraws from it. Both look",
    "like a sudden fall. For drawdown from trading alone, compute it from the",
    "Trades sheet.",
    "",
    "WHY THE PAYOUTS SHEET CAN BE EMPTY",
    "",
    "Payouts published on the Telegram channel belong to archive accounts used",
    "by the payout bot. Those are not client accounts, so — like free signups —",
    "they are outside this statement.",
]
