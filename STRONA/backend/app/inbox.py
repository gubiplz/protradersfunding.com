...
def _zbierz(sciezka: str, marka: str, *, wyslane: bool) -> dict:
    """Wspólne dla odebranych i wysłanych: zbiera z każdego klucza, dzieli na
    marki (odebrane po ODBIORCY, wysłane po NADAWCY), najnowsze pierwsze.
    ...
    for i, klucz in enumerate(klucze()):
        ...
        for m in dane.get("data") or []:
            ...
            if isinstance(do, str):
                do = [do]
            mk = marka_maila([m.get("from") or ""] if wyslane else do, mapa)
            if marka in MARKI and mk != marka:
                continue
            od = parseaddr(m.get("from") or "")[1].lower()
            wynik["items"].append({
                "id": m["id"], "k": i, "brand": mk,
                "from": m.get("from") or "", "from_email": od,
                # Człowiek po naszej stronie rozmowy: przy wysłanym odbiorca,
                # przy odebranym nadawca — po nim dopasowujemy klienta i leada.
                "person_email": (parseaddr(do[0])[1].lower() if do else "") if wyslane else od,
                "to": do, "subject": m.get("subject") or "",
                "created_at": m.get("created_at"),
                "last_event": m.get("last_event") if wyslane else None,
...
def lista(marka: str = "all", *, limit: int = 100) -> dict:
    """Odebrane maile (Resend Receiving), marka po domenie odbiorcy."""
    return _zbierz(f"/emails/receiving?limit={max(1, min(limit, 100))}", marka, wyslane=False)


def wyslane(marka: str = "all", *, limit: int = 100) -> dict:
    """Wysłane przez Resenda (ręczne z panelu i automaty do leadów), ze
    statusem doręczenia `last_event`; marka po domenie nadawcy."""
    return _zbierz(f"/emails?limit={max(1, min(limit, 100))}", marka, wyslane=True)


def jeden(email_id: str, k: int = 0, *, wyslany: bool = False) -> dict:
    """Pełna treść jednego maila (tekst i HTML): odebranego albo wysłanego.
    Rzuca RuntimeError."""
    ks = klucze()
    if not ks:
        raise RuntimeError("RESEND_API_KEY is not set")
    klucz = ks[k] if 0 <= k < len(ks) else ks[0]
    sciezka = "/emails/" if wyslany else "/emails/receiving/"
    m = _http_get(sciezka + urllib.request.quote(email_id, safe=""), klucz)
    do = m.get("to") or []
    if isinstance(do, str):
        do = [do]
    od = parseaddr(m.get("from") or "")[1].lower()
    return {"id": m.get("id") or email_id,
            "brand": marka_maila([m.get("from") or ""] if wyslany else do),
            "from": m.get("from") or "", "from_email": od,
            "person_email": (parseaddr(do[0])[1].lower() if do else "") if wyslany else od,
            "last_event": m.get("last_event") if wyslany else None,
            "to": do, "cc": m.get("cc") or [],
