...
def test_portal_ma_kierunkowe_zanim_ruszy_jego_kod():
    ...
    dane = client.get("/countries.js")
    assert dane.status_code == 200
    assert "immutable" in dane.headers["cache-control"], "bez cache to gorzej niz w HTML-u"
    assert dane.text.startswith("window.PF_GEO="), "bundle czyta wlasnie to okno"
    assert '"PL"' in dane.text and '"48"' in dane.text

    assert "pf-countries" not in html, "lista wrocila do strony"
    assert len(html) < 20_000, f"strona portalu spuchla do {len(html)} B"
